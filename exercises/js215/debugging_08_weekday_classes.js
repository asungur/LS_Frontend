...
const myCalendar = {
  ...
  "2018-08-14": ["Read 'Demystifying Rails'", "Settle health insurance"],  // SINGLE QUOTATION
  "2018-08-15": ["Read 'Demystifying Rails'"],                             // SINGLE QUOTATION
  "2018-08-16": [],                                                       // EMPTY
...
function getCompatibleEvents(classes, calendar) {
  ...
  Object.keys(classes).forEach(className => {
    ...
    if (classDates.filter(isWeekday).every(isAvailable)) {                // why every why not filter only
