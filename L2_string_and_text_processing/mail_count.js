...
function mailCount(emailData) {
  ...
  let dates = emails.map(email => email.split('#/#')[2].slice(7).split('-'));

  let dateObjs = dates.map(date => {
  ...
  let firstDayString = dateObjs[0].toDateString();
  let lastDayString = dateObjs[dateObjs.length - 1].toDateString();

  console.log('Count of Email: ' + emails.length);
  console.log('Date Range: ' + firstDayString + ' - ' + lastDayString);
}
